...
async function pushProduct(ims, economic, eventId, productGroups, economicProduct) {
	...
	if (found) {
		...
		response = await ims.post('productGroups', { productGroupName: productGroupName, itemLabelReportId: 506203569 });
	...
	if (globalTradeItem.globalTradeItemNumber != null) {
...
async function postMessage(ims, detail, text) {
    let message = new Object();
	message.time = Date.now();
	message.source = "economic Integration";
...
exports.documentHandler = async (event, awsContext) => {
    ...
    let economic = await getEconomic();
    let organization = await economic.get('organization');
    
    let response = await economic.get('accounts');
    ...
    if (detail.documentType == 'GOODS_RECEIPT') {
        ...
        transaction.description = "Leverance " + detail.inboundShipmentNumber + " Modtagelseskvittering " + document.documentNumber;

		let transactionLine = new Object();
		transactionLine.accountId = chartOfAccounts.lookup(setup.intermediateAccount);
		...
		transactionLine.accountId = chartOfAccounts.lookup(setup.inventoryAccount);
        transactionLine.amount = Math.abs(document.value);
        ...
        let transactionLine = new Object();
        transactionLine.accountId = chartOfAccounts.lookup(setup.adjustmentAccount).id;
        transactionLine.amount = Math.abs(document.value);
        transactionLine.side = document.value < 0 ? 'debit' : 'credit';
        transactionLines.push(transactionLine);
        ...
        let transactionLine = new Object();
        transactionLine.accountId = chartOfAccounts.lookup(setup.costOfSalesAccount).id;
        transactionLine.amount = Math.abs(document.value);
        transactionLine.side = document.value < 0 ? 'debit' : 'credit';
        transactionLines.push(transactionLine);
        ...
        transaction.description = "Leverance " + detail.inboundShipmentNumber + " Efterberegning " + document.documentNumber;

        let transactionLine = new Object();
        transactionLine.accountId = chartOfAccounts.lookup(setup.intermediateAccount).id;
        transactionLine.amount = Math.abs(document.value);
        transactionLine.side = document.value < 0 ? 'debit' : 'credit';
        transactionLines.push(transactionLine);
        ...
        let transactionLine = new Object();
        transactionLine.accountId = chartOfAccounts.lookup(setup.deprecationAccount).id;
        transactionLine.amount = Math.abs(document.value);
        transactionLine.side = document.value < 0 ? 'debit' : 'credit';
        transactionLines.push(transactionLine);
    ...
    response = await economic.post('daybookTransactions', { daybookTransaction: transaction });
